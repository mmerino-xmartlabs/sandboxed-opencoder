...
3. **Port Collisions (The Gradio/FastAPI Rule):** If you build a web UI, it MUST bind to `0.0.0.0` and dynamically read `APP_PORT` from the `.env` file (fallback to 7860). **CRITICAL:** Before launching a server, kill ghost processes holding the port with `sudo agent-kill-port "${APP_PORT:-7860}"`.
